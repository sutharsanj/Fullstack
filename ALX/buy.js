...
var addToCartButtons = document.querySelectorAll('.details button');
...
function addToCart(event) {
  var buyItem = event.target.parentElement.parentElement; // Adjust the parent element
  var priceText = buyItem.querySelector('p').textContent;
  var price = parseFloat(priceText.match(/\d+\.\d+/)); // Extract the price as a floating-point number

  var item = {
    name: buyItem.querySelector('h3').textContent,
